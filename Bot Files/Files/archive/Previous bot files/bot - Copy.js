...
client.login("NjEyNzQyNjYwNjU4MTY3ODQw.XVmzEg.9bWoo8FGWT9psDFyJHmqnXSPRNY");
client.on('ready', () => {
  console.log("[SUCCESS] Logged in to discord");
  client.user.setActivity("Minecraft // j.help", { type: "PLAYING"}); //Activities: Playing, Watching, Listening (MUST BE IN CAPS) 
});
...
client.on("message", async function (message) {
  ...
  if (message.content.startsWith("j.owo")) { //759914061696860202
    message.channel.send('Whats this?');
    console.log('[MSG] Whats This?');
    return
  }
...
message.channel.send('>>> Added j.card')
...
//message.channel.send(':x: this command is disabled.')
    message.delete().catch(O_o => { }); //deletes previous messages. Sneaky, Sneaky, right?
...
msg.delete();
  ...
  if (message.content.startsWith("j.help")) {
    ...
    var helpb = new Discord.MessageEmbed()
        ...
        .setDescription('')
        .addFields(
        { name: 'xd', value: '**MODERATION COMMANDS**', inline: false},
        ...
        { name: 'j.stickykeys^', value: 'What a great accessiblity feature!', inline: true},
        { name: '\u200B', value: '\u200B' },
        { name: 'this is a name lol', value: '**FUN COMMANDS**', inline: false},
        { name: 'j.8ball [whatever u want]', value: 'Shake the magic 8ball!', inline: true},
        { name: 'j.9ball', value: 'just an old version of the 8ball. nothing really special', inline: true},
        { name: 'j.69ball', value: 'Lws funny ball!!1111', inline: true},
        { name: 'j.yesno', value: 'Only ask a yes or no question, just like the 8ball', inline: true},
        ...
        { name: 'j.ss <message>', value: 'Send a message to our server. **If you post any nsfw, you will be banned from using j.ss without any warning**', inline: true},
        { name: 'j.dice [number]', value: 'Roll the dice!', inline: true},
        { name: 'j.coinflip', value: 'Tails or heads?', inline: true},
        ...
        { name: 'j.td', value: 'Truth or dare?', inline: true},
        { name: 'j.cards', value: 'Get a random card', inline: true},
        { name: 'j.suprise', value: 'is the monstercat show up at the box?', inline: true},
        ) //
        .setTimestamp()
    ...
    var helpa = new Discord.MessageEmbed()
        ...
        .setDescription('Here is the help list of this bot, for your best times on this server!')
        .addFields(
       { name: '1', value: '**GENERAL COMMANDS**', inline: false},
        { name: 'j.help', value: 'Shows the commands that are in this bot.', inline: true},
        { name: 'j.about', value: 'Small piece of information about this bot', inline: true},
        { name: 'j.test', value: 'Tests the bot message thing', inline: true},
        { name: 'j.invite', value: 'Invites the bot to your server. it also gives u an invite to my server', inline: true},
        { name: 'j.ping', value: 'Pong!', inline: true}, //
        { name: 'j.version', value: 'Tells you the version of this stupid bot.', inline: true},
        { name: 'j.servercount', value: 'Tells you how many server this bot is in.', inline: true},
        { name: 'j.reacttest', value: 'Tests the Reactions', inline: true},
        { name: 'j.feedback', value: 'If you want to leave a suggestion / bug report or whatever, then this is for u. If you want to keep it anonymous, Please do j.privatefeedback <feedback>. do not use j.privatefeedback if its a appeal **unless you have a Discord ID with it**, We promise! We wont sell your personal info to anywhere! Not even the dark web. ', inline: true},
        { name: 'j.privatefeedback', value: 'If you want to keep it anonymous, Please do j.privatefeedback <feedback>. do not use j.privatefeedback if its a appeal **unless you have a Discord ID with it**, We promise! We wont sell your personal info to anywhere! Not even the dark web. ', inline: true},
        { name: '\u200B', value: '\u200B' },
        { name: 'j.srules', value: 'Read the j.ss rules', inline: true},
        { name: '2', value: '**TEXT EDIT COMMANDS**', inline: false},
        { name: 'j.uppercase <whatever u want>', value: 'Changes all text to CAPS. [ul can also be used.]', inline: true},
        { name: 'j.lowercase', value: 'Changes all text to lowercase [ls can also be used.]', inline: true},
        { name: 'j.say <text>', value: 'Repeats what you say in the syntax', inline: true},
        { name: 'j.nsay <text>', value: 'Repeats what you say in the syntax, deletes your message afterwards', inline: true},
        { name: 'j.scramble <text>', value: 'Scrambles the text.', inline: true},
        { name: 'j.thot <text or user>', value: 'BEGONE THOT!', inline: true},
        { name: 'j.vaporwave <text>', value: 'S p a c e s  e v e r y  c h a r a c t e r ,  i n  m e s s a g e', inline: true},
        )
    var helpd = new Discord.MessageEmbed()
        ...
        .setDescription('')
        .addFields(
       { name: '4', value: '**ANIMAL COMMANDS**', inline: false},
    ...
    var helpe = new Discord.MessageEmbed()
        ...
        .setDescription('')
        .addFields(
       { name: '5', value: '**ROLEPLAY COMMANDS**', inline: false},
    ...
    var helpx = new Discord.MessageEmbed()
        ...
        .addFields(
        { name: 'uiikghkjh', value: '**UTILITIES COMMANDS**', inline: false},
 ...
 var helpc = new Discord.MessageEmbed()
        ...
        .setDescription('These commands are none of your buiness. Litterally.')
        .addFields(
...
//   if (!faces) return message.channel.send("🎲 " + nanieeggyes)
...
message.channel.send(":x: Sorry, this command is not allowed in NSFW channels.")
    ...
    console.log('[MSG] :heavy_check_mark: Thanks for the feedback! If this is for appeals, we recommend using j.feedback or provide your Discord ID, Do not worry! We will never EVER sell your personal info! -- I promise!');
    return
  }
  ...
  if (message.content.startsWith("j.suprise")) {
var mcboxboi = [':package: ...',':package: *He isnt home*',':package: *Nothing is inside this box. You probably picked up the wrong box...*',':package: *Nope. Not Home.*',':package: *He isnt in there.*',':package:','<:MonsterCatsBox:765741159560511529> Suprise!',':package: *You hear him making music inside this box. Its best not to distract him.*','<:MonsterCatsBox:765741159560511529>']
...
console.log('[MSG] :no_entry: ***You are banned from using Server Send:tm:.***n/ Sorry, But we have to ban you from using the service, If you wanna know why, DM Jacob Workman#8117, If you want to appeal, type j.feedback <Your appeal>, The appeal will be only read by us.');
  ...
  if (message.content.startsWith('j.version')) {
    message.channel.send("v"+ version + "RetroJBOT Carrot Edition")
    console.log("[MSG]"+"v"+ version + "RetroJBOT Carrot Edition")
  }
  ...
  if (message.content.startsWith('j.sban ')) {
    ...
    let person = message.content.slice(7).slice(message.content.split(" ")[0].length + 1)
    ssbans.bans.push(person)
    fs.writeFileSync('ssbans.txt', JSON.stringify(ssbans))
    message.channel.send('Success! Swinged the banned hammer on ' + person + " from using `j.ss <text>` https://media1.tenor.com/images/ae83976e867ebc2722054a632ff045ad/tenor.gif?itemid=11035060");
    return;
  }
  ...
  if (message.content.startsWith("j.stickykeys")) {
    //ADD THE FOLLOWING IF TO **ALL** YOUR ADMIN-ONLY COMMANDS
      if (!message.member.hasPermission("MANAGE_MESSAGES")) {
      message.channel.send(':x: You must have `MANAGE_MESSAGES`, idk why, but you need it.')
      return;
    } else {
    message.delete().catch(O_o => { });
    var helpaa = new Discord.MessageEmbed()
        .setColor('#00b7ff')
        .setTitle('Sticky Keys')
        .setDescription('')
        .addFields(
       {name: ':information_source: Do you want to turn on Sticky Keys?', value: 'Sticky Keys lets you use the SHIFT, CTRL, ALT or RetroJOS keys by pressing one at a time. The keyboard shortcut to turn on Sticky Keys is to press the SHIFT key 5 times. (y/n)', inline: false},
       )

    message.channel.send(helpaa);
  }
...
message.channel.send(ojchan[Math.floor(Math.random() * ojchan.length)]);
  ...
  if (message.content.startsWith("j.shrek")) {
    message.channel.send(':x: This command is disabled. There is a bug inside of it that I am trying to fix. :(');
  }
  if (message.content.startsWith("j.winxp")) {
    message.channel.send(':x: This command is disabled. There is a bug inside of it that I am trying to fix. :(');
  }
  });
...
client.on('message', async message => {
  ...
  if (message.content.startsWith('j.die')) {
    ...
    if (!adminlist.admins.includes(message.author.id)) {
      ...
      try {
        console.log('[MSG] [Client.destroy] Shutting Down...')
        message.channel.send(":warning: Shutting Down...");
        client.destroy()
      } catch (error) {
        message.reply(':x: Failed to shutdown. Error in the terminal.');
        console.log('[MSG] [CRITERROR] :X: Failed to shutdown: ')
