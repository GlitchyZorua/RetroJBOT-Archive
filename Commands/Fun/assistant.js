...
const unixTime = Math.floor(Date.now() / 1000);
 ...
 var embed = new Discord.MessageEmbed()
  ...
  if (thingtosend.toLowerCase() == "how far is the sun") {
    ...
    require('https').get('https://api.wolframalpha.com/v1/result?i='+encodeURIComponent(thingtosend)+'&appid=G3UG26-V9K9QVY5JA', function (d) { d.on('data', function (n) { message.channel.send('`'+String(n)+'`') }) })
return;
}
    }
}