...
var disableddms = false;
var disableinteractions = false;
var logmode = false;
module.exports = (client, message) => {
    ...
    if (!message.channel.type === 'dm' && serverbans.es.includes(message.guild.id)) {
    ...
    const command = client.commands.get(cmdCall) ?? client.commands.find(cmd => cmd.aliases?.includes(cmdCall));
    const args = message.content.split(' ').slice(1);
    if (RegExp(`^(<@!?${client.user.id}>)\\s*`).test(message.content)) args = args.slice(1);
    if (command) {
